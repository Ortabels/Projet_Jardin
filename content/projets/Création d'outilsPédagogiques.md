---
title: "Création d’outils pédagogiques Sol vivant & Agroécologie"
date: 2025-11-26
description: "L’association organise un chantier bénévole pour fabriquer une série d’outils pédagogiques manipulables (puzzles, tests, maquettes) permettant d’expliquer aux enfants le fonctionnement du sol vivant, la faune du sol, et les chaînes alimentaires souterraines.
Ces outils seront ensuite utilisés lors d’interventions éducatives auprès des écoles, centres de loisirs et structures de jeunesse du territoire."
tags: ["Pédagogie", "tag"]
authors: "Jérémy"
---

Chantier participatif : fabrication de puzzles pédagogiques pour comprendre le sol vivant

---

### 🎓 Objectifs pédagogiques (pour les futurs ateliers)
- Comprendre les horizons du sol et leur rôle.
- Identifier les organismes du sol (décomposeurs, prédateurs, ingénieurs du sol).
- Visualiser les chaînes alimentaires du sol grâce à un puzzle dédié.
- Observer la décomposition en direct via un vermicomposteur vitré.
- Sensibiliser aux principes de l’agroécologie : paillage, compostage, cycles naturels.

---

### 🔧 Outils pédagogiques à fabriquer :

#### 🧩 1. Puzzle “Horizons du sol”
Inspiré du module “Horizons sous nos pieds” :
6 à 7 pièces (litière, humus, horizon organo-minéral, sous-sol…).
- Format A4 max, permettant une manipulation en groupe.

#### 🐛 2. Puzzle “Qui vit dans le sol ?” :
Inspiré du “Qui vit là-dessous ?” 
- Silhouettes d’animaux (vers, cloportes, collemboles…)
- Cartes explicatives associées.

#### 🧩 3. Puzzle “Chaîne alimentaire du sol” :
Pièces représentant :
débris végétaux
champignons / bactéries
décomposeurs
prédateurs du sol
plante / racines

#### 🧪 4. Série de “Tests du bocal” :
Bocaux en verre remplis de sols différents :
- sableux
- argileux
- limoneux
- sol du potager (à deviner !)
Décantation + observation des couches → texture & structure.

#### 🪱 5. Vermicomposteur vitré :
Petit modèle (50–60 cm de haut).
- Plexiglas + bois ou carton rigide consolidé.
- Observation : vers, couches de matière, évolution dans le temps.

---

### 👥 Participants à la fabrication :
- Bénévoles de l’association
- Adhérents
- Parents / habitants intéressés
- Étudiants, volontaires, membres du CA

---

### 🧒 Public bénéficiaire final (après fabrication) :
- Classes CP → CM2
- Centres de loisirs
- Périscolaire / ALSH
- Associations jeunesse
- Événements familles / nature

---

### 📅 Organisation du chantier :
- Phase 1 – Préparation (1 semaine)
Achat / récupération du matériel
Impression des visuels
Découpe des formats bruts

- Phase 2 – Fabrication collective (1 journée)
Ateliers simultanés :
Puzzle horizons du sol
Puzzle faune du sol
Puzzle chaînes alimentaires
Préparation des bocaux
Assemblage du vermicomposteur

- Phase 3 – Finitions (1–3 jours)
Vernis écologique
Plastification des cartes
Assemblage final
Test d’usage / solidité

---

### 🧰 Matériel nécessaire :
**Pour les puzzles**
- Planche de récup'
- Outil de coupe et de travail du bois 
- Feutres, peinture non toxique
- Vernis écologique

**Pour les tests du bocal**
- Bocaux en verre (x4)
- Sols variés + échantillon du potager
- Étiquettes

**Pour le vermicomposteur**
- Plexiglas 2–3 mm (petit format)
- Bois fin / carton renforcé
- Vers Eisenia
- Compost / litière
- Fiches pédagogiques imprimées
- Petits outils (loupes, pinceaux…)

---

### 💶 Budget prévisionnel :
- Matériel puzzles	80–120 €
- Matériel vermicomposteur	50–80 €
- Bocaux & fournitures	20–40 €
- Impressions / plastification	30–50 €
- Consommables (colles, feutres…)	20–40 €
- Total matériel	200–330 €
- Valorisation du temps bénévole	offert
- Budget global demandé	200–350 €

---

### 🌍 Retombées du projet :
- Outils réutilisables pendant plusieurs années.
- Mise en place d’animations de qualité professionnelle.
- Forte visibilité de l’association auprès des écoles.
- Sensibilisation durable aux thèmes : sol vivant, biodiversité, décomposition.
- Création d’un socle éducatif aligné avec les programmes scolaires.

---

### ✔️ Points forts pour la commission :
Projet à bas coût avec impact massif.
Outils ludiques + approche scientifique dès le CP.
Mobilisation citoyenne / bénévolat.
Complètement aligné avec les enjeux écologiques actuels.
Projet duplicable sur plusieurs années.
